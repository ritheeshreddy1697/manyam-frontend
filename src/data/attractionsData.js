import templeImg from "../assets/images/temples/temple1.jpeg";
import waterfallImg from "../assets/images/waterfalls/waterfall1.jpeg";
import viewpointImg from "../assets/images/viewpoints/viewpoint1.jpeg";
import festivalImg from "../assets/images/viewpoints/tribal1.jpeg";

export const attractionsData = [
  {
    title: "Ancient Temples",
    category: "Temples",
    description: "Historic temples with rich architecture and heritage.",
    image: templeImg,
    link: "/attractions/temples",
  },
  {
    title: "Scenic Waterfalls",
    category: "Waterfalls",
    description: "Hidden waterfalls surrounded by lush green forests.",
    image: waterfallImg,
    link: "/attractions/waterfalls",
  },
  {
    title: "Hill Viewpoints",
    category: "Viewpoints",
    description: "Breathtaking panoramic hill views.",
    image: viewpointImg,
    link: "/attractions/viewpoints",
  },
  {
    title: "Festivals & Culture",
    category: "Festivals",
    description: "Colorful local festivals reflecting rich traditions.",
    image: festivalImg,
    link: "/attractions/festivals",
  },
];
